...
const (
	...
	snapshotterNamePrefix      = "snapshotNamePrefix"
)
...
func (r *ReconcileStorageCluster) createSnapshotClasses(vscs []*snapapi.VolumeSnapshotClass, reqLogger logr.Logger) error {
	for _, vsc := range vscs {
		...
		errMsg := ""
		if err != nil {
			errMsg = "Failed to 'Get' the SnapshotClass"
			if errors.IsNotFound(err) {
				// Since the SnapshotClass is not found, we will create a new one
				reqLogger.Info(fmt.Sprintf("Creating SnapshotClass: %s", vsc.Name))
				if err = r.client.Create(context.TODO(), vsc); err == nil {
					// no error then continue to check the next snapshotter
					continue
				}
				errMsg = "Failed to 'Create' the SnapshotClass"
			} // on any error, including (above) resource creation error, will be dealt below
		}
		// if there is *still* an error, we should report it
		if err != nil {
			reqLogger.Error(err, fmt.Sprintf("%s: %s", errMsg, vsc.Name))
			return err
		}
